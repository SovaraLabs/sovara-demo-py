...
def build_coarse_toc(page_list, start_index=1, max_pages_each_node=None):
    if max_pages_each_node is None:
        max_pages_each_node = 10
    max_pages_each_node = max(1, int(max_pages_each_node))

    toc_items = []
    end_index = start_index + len(page_list) - 1
    structure_index = 1
    for page_index in range(start_index, end_index + 1, max_pages_each_node):
        page_end = min(page_index + max_pages_each_node - 1, end_index)
        if page_index == page_end:
            title = f"Page {page_index}"
        else:
            title = f"Pages {page_index}-{page_end}"
        toc_items.append({
            "structure": str(structure_index),
            "title": title,
...
################### check title in page #########################################################
async def check_title_appearance(item, page_list, start_index=1, model=None, reasoning_effort=None):    
    title=item['title']
    ...
    Directly return the final JSON structure. Do not include explanations or reasoning."""
...
async def check_title_appearance_in_start(title, page_text, model=None, logger=None, reasoning_effort=None):    
    prompt = f"""
...
async def check_title_appearance_in_start_concurrent(structure, page_list, model=None, logger=None, reasoning_effort=None):
    if logger:
...
def toc_detector_single_page(content, model=None, reasoning_effort=None):
    prompt = f"""
    ...
    response = llm_completion(model=model, prompt=prompt, reasoning_effort=reasoning_effort)
...
def check_if_toc_extraction_is_complete(content, toc, model=None):
    ...
    prompt = prompt + '\n Document:\n' + content + '\n Table of contents:\n' + toc
    response = llm_completion(model=model, prompt=prompt, reasoning_effort="none")
    json_content = extract_json(response)
...
def check_if_toc_transformation_is_complete(content, toc, model=None):
    ...
    Only judge completeness for actual table-of-contents entries. If the raw text includes
    prose or document body content after the table of contents ends, ignore that trailing
    non-TOC content.
...
def detect_page_index(toc_content, model=None, reasoning_effort=None):
...
def toc_extractor(page_list, toc_page_list, model, reasoning_effort=None):
    ...
    has_page_index = detect_page_index(
        toc_content,
        model=model,
        reasoning_effort=reasoning_effort,
    )
...
def _is_toc_item_list(value):
    return isinstance(value, list) and all(isinstance(item, dict) for item in value)


def _extract_table_of_contents_items(payload):
    if _is_toc_item_list(payload):
        return payload

    if not isinstance(payload, dict):
        raise ValueError(f"Expected TOC JSON object or list, got {type(payload).__name__}")
    ...
    for key in (
        ...
        if _is_toc_item_list(value):
            return value

    list_values = [value for value in payload.values() if _is_toc_item_list(value)]
    if len(list_values) == 1:
        return list_values[0]
    if len(list_values) > 1:
        return [item for value in list_values for item in value]

    raise ValueError("TOC transformation JSON did not include a list of TOC entries")


def _parse_transformed_toc(content):
    payload = extract_json(content) if isinstance(content, str) else content
    toc_items = _extract_table_of_contents_items(payload)
    if not toc_items:
        raise ValueError("TOC transformation returned no entries")
    return convert_page_to_int(toc_items)
...
def toc_transformer(toc_content, model=None):
    ...
    The given text may include non-TOC prose or body text after the table of contents ends. Include only actual table-of-contents entries and ignore trailing non-TOC content.
    ...
    if if_complete == "yes" and finish_reason == "finished":
        return _parse_transformed_toc(last_complete)
    ...
    while not (if_complete == "yes" and finish_reason == "finished"):
        ...
        Continue only actual table-of-contents entries. If the raw text includes prose or document body content after the table of contents ends, ignore that trailing non-TOC content.
        The response should be in the following JSON format: 
...
async def fix_incorrect_toc(
    ...
    async def process_and_check_item(incorrect_item):
        ...
        if prev_correct > next_correct:
            nearby_pages = [prev_correct, next_correct]
            item_page = incorrect_item.get('physical_index') or incorrect_item.get('page_number')
            if item_page is not None:
                nearby_pages.append(item_page)
            prev_correct = max(start_index, min(nearby_pages) - 2)
            next_correct = min(end_index, max(nearby_pages) + 2)
...
async def fix_incorrect_toc_with_retries(
    ...
    while current_incorrect:
        ...
        current_toc, current_incorrect = await fix_incorrect_toc(
            ...
            check_reasoning_effort=check_reasoning_effort,
        )
...
################### verify toc #########################################################
async def verify_toc(page_list, list_result, start_index=1, N=None, model=None, reasoning_effort=None):
    ...
    # Early return only if we don't have valid physical indices. A low last
    # start page is valid for documents whose final section spans many pages.
    ...
    results = []
    with tqdm(total=len(tasks), desc="verify TOC items", unit="item") as progress:
...
async def meta_processor(page_list, mode=None, toc_content=None, toc_page_list=None, start_index=1, opt=None, logger=None):
    ...
    if accuracy > 0.6 and len(incorrect_results) > 0:
        toc_with_page_number, incorrect_results = await fix_incorrect_toc_with_retries(
            ...
            start_index=start_index,
            max_attempts=3,
            model=opt.healing_model or opt.model,
            logger=logger,
            check_model=opt.cheap_model,
            check_reasoning_effort="none",
        )
        return toc_with_page_number
    else:
        if mode == 'process_toc_with_page_numbers':
            ...
            raise Exception(PROCESSING_FAILED_MESSAGE)
...
async def process_large_node_recursively(node, page_list, opt=None, logger=None):
    ...
    if node['end_index'] - node['start_index'] > opt.max_page_num_each_node and token_num >= opt.max_token_num_each_node:
        ...
        try:
            ...
            if str(exc) != PROCESSING_FAILED_MESSAGE:
                raise
            if logger:
                logger.info({
                    "message": "Falling back to coarse page ranges for large node",
                    "title": node.get("title"),
                    "start_index": node.get("start_index"),
                    "end_index": node.get("end_index"),
                    "token_num": token_num,
                })
            node_toc_tree = build_coarse_toc(
                node_page_list,
                start_index=node['start_index'],
                max_pages_each_node=opt.max_page_num_each_node,
            )
            fallback_used = True
...
async def tree_parser(page_list, opt, doc=None, logger=None):
    ...
    try:
        ...
        if logger:
            logger.info({
                "message": "Falling back to coarse page ranges for document",
                "total_page_number": len(page_list),
            })
        toc_with_page_number = build_coarse_toc(
            page_list,
            start_index=1,
            max_pages_each_node=opt.max_page_num_each_node,
        )
...
def page_index(doc, model=None, cheap_model=None, healing_model=None, toc_check_page_num=None, max_page_num_each_node=None, max_token_num_each_node=None,
